class SnippetStore:
    def __init__(self):
        ...
        self.UDP = {
            ...
            "first_para": "UDP scanning in the framework is performed using the Get-NetUDPEndpoint command. This command collects active UDP endpoints from the system, including the local address, local port, and the associated process ID (PID). Since UDP is a connectionless protocol, it does not maintain session states like TCP, so identifying listening UDP ports helps determine which services are currently using UDP communication.",
            "second_para": "The collected UDP endpoint information acts as a live network artifact for the investigation. By examining the listening ports and their associated processes, investigators can identify unusual or suspicious services that may indicate unauthorized applications or potential malware activity on the system.",
