...
class Forensic_tool:
    ...
    def checking_request(self,api):
        ...
        print(f" \nThe current API key that is present for Hybrid Analysis : {api}")
        try:
    ...
    def tool_requirements(self):
        ...
        while True:
            answer = input("\nDo u want to save the recent hashes (yes or no) : ").lower()
            if answer == "yes":
                self.save_hash = True
                break
            if answer == "no":
    ...
    def output_dir(self):
        ...
        self.pre_tools_path = f"{self.cwd}\\forensic_tools"   #######        self.pre_tools_path = f"{self.cwd}\\_internal\\forensic_tools"   #######
...
system_info.powershell_log()
First_module(forensic_run.sysinfo,forensic_run.report_path,forensic_run.real_evidence).html_method()
