...
class Browser_artifacts:
    ...
    def edge_data(self):
        ...
        for i in self.pc_users:
            original_edge_dir = fr"C:\Users\{i}\AppData\Local\Microsoft\Edge\User Data\Default\History"
            edge_dir = fr"{self.unfiltered_evidence}\\Edge_History_{i}"

            if not os.path.exists(original_edge_dir):
                continue
            shutil.copyfile(original_edge_dir,edge_dir)
            try:
                ...
                print(f"{e} \n Close the Edge Browser and Try again")
                return
    ...
    def firefox_artifacts(self):
        ...
        if not os.path.exists(self.filtered_evidence):
            firefox_datas.append(["Downloaded URL", "Destination file Location","Date", "Browser", "User"])
        ...
        for sql_file in set_fire_profile:
            ...
            try:
                ...
                print(f"{e} \n Close the FireFox Browser and Try again")
                return
    ...
    def chrome_artifacts(self):
        chrome_datas = []
        if not os.path.exists(self.filtered_evidence):
            chrome_datas.append(["Downloaded URL", "Destination file Location", "Date", "Browser", "User"])
        for i in self.pc_users:
            org_chrome_dir = fr"C:\Users\{i}\AppData\Local\Google\Chrome\User Data\Default\History"
            chrome_dir = fr"{self.unfiltered_evidence}\\History_{i}"
            if not os.path.exists(org_chrome_dir):
                continue
            shutil.copyfile(org_chrome_dir,chrome_dir)
            try:
